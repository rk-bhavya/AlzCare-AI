...
export const registerPatient = async (req, res) => {
  try {
     ...
     *
     * Captured as name + relationship only at registration time
     * (no photo here — this is a public, unauthenticated
     * endpoint, so photo/face-reference data is intentionally
     * only added later by an authenticated caregiver via the
     * Patient Details page). This is best-effort: a malformed
     * or missing familyMembers field never blocks account
     * creation.
     */
    ...
    if (familyMembers) {
      try {
        ...
        if (Array.isArray(parsedFamilyMembers)) {
          const validMembers = parsedFamilyMembers
            .filter(
              (member) =>
                member &&
          ...
          if (validMembers.length > 0) {
            await FamilyMember.insertMany(validMembers);
          }
        }
      } catch (familyMemberError) {
        console.error(
          "Unable to save family members during patient registration:",
          familyMemberError
        );
      }
    }
