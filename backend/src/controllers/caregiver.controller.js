...
/* ============================================================
   BUILD COGNITIVE PROGRESS SUMMARY

   Shared helper used by Patient Monitoring to summarize a
   patient's cognitive activity history (the same data source
   used by the existing CognitiveActivity backend / games).
   Purely derives a summary from real documents — never invents
   values.
============================================================ */
...
export const buildCognitiveProgress = (activities = []) => {
  ...
  const latestEntry = activities[0];

  const latest = {
    activityType: latestEntry.activityType,
    label:
      ACTIVITY_LABELS[latestEntry.activityType] || latestEntry.activityType,
    score: latestEntry.score,
    durationSeconds: latestEntry.durationSeconds || null,
  ...
  activities.forEach((activity) => {
    if (seenTypes.has(activity.activityType)) return;
    seenTypes.add(activity.activityType);
...
export const getCaregiverPatientDetails = async (req, res) => {
  try {
    ...
    const [assessments, medications, appointments, alerts, familyMembers] =
      await Promise.all([
    ...
    const monitoring = await Promise.all(
      assignments
        ...
        .map(async (assignment) => {
          ...
          const recentActivities = await CognitiveActivity.find({
            patientId,
          })
            .sort({ createdAt: -1 })
            .limit(20)
          ...
          const cognitiveProgress = buildCognitiveProgress(
            recentActivities
          );
          ...
          const dailyTaskProgress = {
            ...
            pending: dailyTasks.length - completedTaskCount,
          };

          const patientDevice = await PatientDevice.findOne({
            patientId,
            status: "active",
          })
            .select("lastKnownLocation lastActiveAt")
            .lean();

          const location =
            patientDevice?.lastKnownLocation?.updatedAt
              ? {
                  status: "available",
                  latitude: patientDevice.lastKnownLocation.latitude,
                  longitude: patientDevice.lastKnownLocation.longitude,
                  updatedAt: patientDevice.lastKnownLocation.updatedAt,
                }
              : {
                  status: "unavailable",
          ...
          const helpAlerts = await Notification.find({
            recipientId: req.user._id,
            patientId,
          ...
          const faceRecognitionEvents = await FaceRecognitionEvent.find({ patientId })
            .populate({ path: "familyMemberId", select: "name relationship" })
            .sort({ createdAt: -1 })
            .limit(5)
            .lean();
