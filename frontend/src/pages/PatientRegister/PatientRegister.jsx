...
const PatientRegister = () => {
  ...
  const handleAddFamilyMember = () => {
    setFamilyMembers((previous) => [
      ...previous,
      { id: `fm_${Date.now()}_${previous.length}`, name: "", relationship: "" },
    ]);
  };
  ...
  const validateForm = () => {
    ...
    familyMembers.forEach((member) => {
      ...
      const hasRelationship = member.relationship.trim();

      if (hasName && !hasRelationship) {
        newErrors[`familyMember_${member.id}`] =
          "Please select a relationship.";
      } else if (!hasName && hasRelationship) {
        newErrors[`familyMember_${member.id}`] =
          "Please enter a name.";
      }
    });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const completeFamilyMembers = familyMembers
        .filter(
          (member) =>
      ...
      if (completeFamilyMembers.length > 0) {
        data.append(
          "familyMembers",
          JSON.stringify(completeFamilyMembers)
        );
      }
            ...
            <div className="form-grid">
              ...
              <div className="form-section-title form-section-title-grid">
                ...
                <span className="form-section-subtitle">
                  Optional · helps your care team recognize the people around you
                </span>
              </div>

              <div className="form-group full-width family-members-section">
                ...
                {familyMembers.map((member, index) => (
                  <div className="family-member-row" key={member.id}>
                    <div className="family-member-row__fields">
                      <div className="family-member-field">
                        <label>Name</label>
                        <input
                          type="text"
                    ...
                    {errors[`familyMember_${member.id}`] && (
                      <span className="field-error">
                        {errors[`familyMember_${member.id}`]}
                      </span>
                    )}
                ...
                <p className="family-members-hint">
                  You can add photos for each family member later from your
                  caregiver's Patient Details page.
                </p>
              </div>
